...
interface ItemToClearDeliveryStatus {
  jobNumber: string;
  listNumber: string;
  partNumber: string;
}
...
/**
 * POST /api/admin/orders/clear-delivery-status
 * Clears delivery/pickup transit status and received state for selected ordered lines.
 * Acts as undo for Mark Received, Mark Pickup, and Mark Delivery to Jobsite.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const auth = await requirePermission(session, 'orders.clear_delivery_status');
    if (!auth.ok) return auth.response;

    const body = await request.json();
    const { items } = body as { items: ItemToClearDeliveryStatus[] };
    ...
    console.error('Error in /api/admin/orders/clear-delivery-status:', error);
    return NextResponse.json(
